...
#define LOG(level, source, format, ...)                                        \
  ndsl_log_into_sink(level, source, __FILE__, __FUNCTION__, format,            \
                     ##__VA_ARGS__)
...
// 输出选项
//
enum { LOG_OUTPUT_TER = 0, LOG_OUTPUT_FILE = 1 };

void set_ndsl_log_sinks(uint64_t sinks, int file_or_ter);
void ndsl_log_into_sink(int level, uint64_t source, const char *file_name,
